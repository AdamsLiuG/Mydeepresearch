"""CLI entrypoint for CPU and memory profiling benchmarks."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the engineering profile benchmark.")
    parser.add_argument("--profile", default="stub", choices=["stub", "real_local"])
    parser.add_argument("--scenario", default=None, help="Optional path to a profiling scenario JSON file.")
    ...
    parser.add_argument(
        "--tracemalloc",
        action="store_true",
        help="Capture the benchmark runner tracemalloc peak as an additional field.",
    )
...
def main() -> int:
    args = parse_args()
    scenario = load_scenario("profile.json", args.scenario)
    payload = execute_http_benchmark(
        mode="profile",
        profile=args.profile,
        scenario=scenario,
        output_path=resolve_output_path("profile", args.profile, args.output),
